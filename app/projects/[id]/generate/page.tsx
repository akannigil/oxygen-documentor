...
function getTemplateType(template: Template): string {
  ...
  if (template.mimeType) {
    ...
    if (template.mimeType === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') return 'docx';
  }
      ...
      try {
        ...
        if (res.ok) {
          ...
          const templatesData = await Promise.all(
            (data.templates || []).map(async (t: { id: string; name: string; description: string | null; fields: unknown; templateType?: string; variables?: unknown; width?: number; height?: number }) => {
              try {
                ...
                if (templateRes.ok) {
                  ...
                  const loadedTemplate = {
                    ...
                    fields: Array.isArray(templateData.fields || t.fields) ? (templateData.fields || t.fields) : [],
                    variables: Array.isArray(templateData.variables || (t as any).variables) ? (templateData.variables || (t as any).variables) : undefined,
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="mb-8">
          <Link href={`/projects/${projectId}`} className="text-sm font-medium text-blue-600 hover:text-blue-500">
            ← Retour au projet
          </Link>
        ...
        <div className="mb-8 rounded-lg bg-white p-6 shadow-sm">
            ...
            {templates.map((t) => (
              ...
              >
                ...
                {(() => {
                  const type = getTemplateType(t);
                  if (type === 'docx') {
                    return (t.variables?.length || 0) === 0 && (
                      <div className="mt-2 text-xs text-yellow-600">
                        ⚠️ Aucune variable détectée
                      </div>
                    );
                  }
                  return (t.fields?.length || 0) === 0 && (
                    <div className="mt-2 text-xs text-yellow-600">
                      ⚠️ Aucune zone définie
                    </div>
                  );
                })()}
              </button>
          ...
          {templates.length === 0 && (
            <div className="text-center py-8">
              <p className="text-sm text-gray-600 mb-4">Aucun template disponible</p>
              <Link
