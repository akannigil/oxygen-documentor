...
export async function POST(request: Request, { params }: RouteParams) {
  try {
    ...
    const bodySchema = z.object({
      ...
      rows: z
        .array(z.record(rowValueSchema))
        .min(1, 'rows requis')
        .max(500, 'Taille maximale 500 lignes par requête'),
    ...
    if (!template) {
      return NextResponse.json(
        { error: "Template non trouvé ou n'appartient pas à ce projet" },
        { status: 404 }
      )
    }
    ...
    const jobData = {
      ...
      rows,
      userId,
      ...(outputFormat === 'docx' || outputFormat === 'pdf' ? { outputFormat } : {}),
      ...(pdfOptions ? { pdfOptions } : {}),
      ...(styleOptions ? { styleOptions } : {}),
    } as DocumentGenerationJobData

    const job = await documentGenerationQueue.add('generate-documents', jobData, {
    ...
    return NextResponse.json(
      {
        error: 'Une erreur est survenue lors du lancement de la génération',
