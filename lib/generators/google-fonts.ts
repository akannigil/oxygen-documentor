...
export async function downloadGoogleFont(
  ...
  try {
    ...
    if (!cssResponse.ok) {
      throw new Error(
        `Impossible de récupérer la police ${fontFamily} depuis Google Fonts (HTTP ${cssResponse.status})`
      )
    }
    ...
    if (!fontResponse.ok) {
      throw new Error(
        `Impossible de télécharger le fichier de police depuis ${fontUrl} (HTTP ${fontResponse.status})`
      )
    }
...
<w:fonts xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
    ...
    if (!contentTypesXml.includes(`fonts/${fontFileName}`)) {
      ...
      if (contentTypesXml.includes('</Types>')) {
        contentTypesXml = contentTypesXml.replace('</Types>', `${fontReference}\n</Types>`)
      } else {
