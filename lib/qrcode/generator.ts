...
export function formatQRCodeContent(content: QRCodeContent): string {
  switch (content.type) {
    ...
    case 'email':
      ...
      const subject = content.data['subject'] ? `?subject=${encodeURIComponent(String(content.data['subject']))}` : ''
      const body = content.data['body'] ? `${subject ? '&' : '?'}body=${encodeURIComponent(String(content.data['body']))}` : ''
    ...
    case 'sms':
      ...
      const message = content.data['message'] ? `?body=${encodeURIComponent(String(content.data['message']))}` : ''
