...
export async function generateQRCodeContent(options: QRCodeWorkflowOptions): Promise<string> {
  ...
  if (field.qrcodeAuth?.enabled && authConfig) {
    ...
    // Construire la config en évitant de passer explicitement undefined
    const expiresIn = field.qrcodeAuth.expiresIn ?? authConfig.expiresIn
  ...
  if (field.qrcodeStorageUrl?.enabled && documentFilePath && getStorageUrl) {
    ...
    const useSigned =
      field.qrcodeStorageUrl.urlType === 'signed' || field.qrcodeStorageUrl.urlType === undefined
...
function buildCertificateData(
  ...
  const grade = getFieldValue(fields.grade)
 ...
 */
function getFieldValue(field: TemplateField, data: Record<string, string | number | Date>): string {
...
export async function generateQRCodeWithOptions(
  ...
  if (field.qrcodeOptions?.color !== undefined) {
    options.color = field.qrcodeOptions.color
  }
