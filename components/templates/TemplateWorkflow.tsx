...
export function TemplateWorkflow({ template: initialTemplate }: TemplateWorkflowProps) {
  ...
  const [localQrcodeConfigs, setLocalQrcodeConfigs] = useState<DOCXQRCodeConfig[]>((template.qrcodeConfigs as unknown as DOCXQRCodeConfig[]) ?? [])
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        const parsed = JSON.parse(raw) as MailDefaults & { columnMapping?: { recipient_name?: string; recipient_email?: string } }
      ...
      <div className="mt-6">
            ...
            {isEditingQRCodes ? (
                ...
                <div className="grid gap-6 lg:grid-cols-2">
                    ...
                    <div className="rounded-lg bg-white p-6 shadow-sm lg:col-span-2">
                      <h2 className="text-lg font-semibold text-gray-900 mb-4">Mailing — Configuration par défaut</h2>
                      <div className="grid grid-cols-1 gap-4">
                        <div>
                          <label className="block text-sm font-medium text-gray-700">Objet par défaut</label>
                          <input
                            type="text"
                        ...
                        <div>
                          ...
                          <p className="mt-1 text-xs text-gray-500">
                            Variables disponibles: <code>{'{{recipient_name}}'}</code>, <code>{'{{recipient_email}}'}</code>, <code>{'{{template_name}}'}</code>, <code>{'{{project_name}}'}</code>, etc.
                          </p>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div>
                            <label className="block text-sm font-medium text-gray-700">Adresse de réponse par défaut (Reply-To)</label>
                            <input
                              type="email"
                        ...
                        <div className="rounded-md bg-blue-50 p-3 text-xs text-blue-900 mb-4">
                          <p className="font-semibold mb-2">Variables disponibles:</p>
                          <ul className="list-disc list-inside space-y-1">
                            <li><code>{'{{recipient_name}}'}</code> — Nom du destinataire</li>
                            <li><code>{'{{recipient_email}}'}</code> — Email du destinataire</li>
                            <li><code>{'{{template_name}}'}</code> — Nom du template</li>
                            <li><code>{'{{project_name}}'}</code> — Nom du projet</li>
                          </ul>
                        </div>
                        <div className="rounded-md bg-yellow-50 p-4 border border-yellow-200">
                          <h3 className="text-sm font-semibold text-yellow-900 mb-3">Mapping des colonnes CSV vers les variables mailing</h3>
                          <p className="text-xs text-yellow-800 mb-3">
                            Configurez ici le mapping des colonnes de votre fichier CSV/Excel vers les variables mailing. 
                            Si vos colonnes CSV ont des noms différents (ex: &quot;email&quot; au lieu de &quot;recipient_email&quot;), 
                            indiquez le nom exact de la colonne dans votre fichier CSV / Excel.
                          </p>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                              <label className="block text-sm font-medium text-gray-700 mb-1">
                                Colonne CSV pour <code className="text-xs bg-gray-100 px-1 rounded">{'{{recipient_name}}'}</code>
                              </label>
                              <input
                                type="text"
                                ...
                                className="mt-1 w-full rounded-md border border-gray-400 px-3 py-2 text-sm text-gray-900 placeholder-gray-600 focus:border-blue-600 focus:outline-none focus:ring-1 focus:ring-blue-600"
                              />
                              <p className="mt-1 text-xs text-gray-500">Laissez vide si votre colonne s&apos;appelle déjà &quot;recipient_name&quot;</p>
                            </div>
                            <div>
                              <label className="block text-sm font-medium text-gray-700 mb-1">
                                Colonne CSV pour <code className="text-xs bg-gray-100 px-1 rounded">{'{{recipient_email}}'}</code>
                          ...
                          <div className="mt-3 rounded-md bg-blue-50 p-2 text-xs text-blue-900">
                            ...
                            <p>
                              Lors de la génération des documents, le système cherchera automatiquement les colonnes CSV 
                              mappées ici pour remplir les variables <code>recipient_name</code> et <code>recipient_email</code>. 
                              Si le mapping n&apos;est pas configuré, il cherchera des colonnes nommées exactement &quot;recipient_name&quot; et &quot;recipient_email&quot;.
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center gap-3 text-sm">
                          <Link href={`/projects/${template.projectId}/templates/${template.id}/send`} className="inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 font-semibold text-gray-700 hover:bg-gray-50">
                            Configurer l’envoi en masse →
                          </Link>
                          <span className="text-gray-400">(utilise les valeurs par défaut ci-dessus si non précisé)</span>
                          <button onClick={handleSaveMailDefaults} disabled={isSavingMail} className="ml-auto inline-flex items-center rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-500 disabled:opacity-50">
                            {isSavingMail ? 'Enregistrement...' : 'Enregistrer dans le template'}
                          </button>
                        </div>
                        {saveMailError && <p className="text-sm text-red-600 mt-1">{saveMailError}</p>}
                        {saveMailSuccess && <p className="text-sm text-green-700 mt-1">Configuration enregistrée</p>}
                      </div>
                    </div>
                </div>
            )}
        ...
        {activeTab === 'documents' && (
          <DocumentList
            ...
            {...(emailBccDefault ? { defaultBcc: emailBccDefault.split(',').map(e => e.trim()).filter(e => e) } : {})}
          />
        )}
      </div>
    </div>
