...
export function VisualPreview({ template, previewData }: VisualPreviewProps) {
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 rounded-md bg-blue-50 p-3">
            <p className="text-xs text-blue-800">
              💡 Les variables seront remplacées dans votre document Word tout en conservant la mise
              en forme, les polices et les styles définis dans le template.
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Canvas d'aperçu */}
      <div
        className="relative mx-auto rounded-lg border border-gray-300 bg-gray-50 shadow-lg"
        ...
        <div className="absolute inset-0">
          {template.fields?.map((field, index) => {
            ...
            return (
              ...
              >
                {field.type === 'qrcode' ? (
                  <div className="flex h-full w-full items-center justify-center border border-gray-400 bg-white">
                    <svg className="h-full w-full p-1" viewBox="0 0 24 24" fill="currentColor">
            ...
            return (
              <div key={`field-detail-${index}`} className="rounded-md border border-gray-200 p-3">
                ...
                >
                  {field.type === 'qrcode' ? (
                    <span className="text-xs text-gray-500">
                      QR: {String(value).substring(0, 30)}...
                    </span>
                  ) : (
                    String(value || '(vide)')
                  )}
